...
console.log('parseTypes(result) = ' + JSON.stringify(self.parse(result)));
            ...
            if (mode === 'array') {
              ...
              if (Array.isArray(item)) {
                ...
                for (var j = 0; j < item.length; j++) {
                  ...
                  else result.push(self.parse(item[j]));
                }
              } else {
                result = (typeof item === 'object' ? [self.parse(item)] : [item]);
              }
              ...
              else result[key] = self.parse(item);
